...
import numpy as np
data = pickle.load(open(os.path.join(DATA_PATH, "02-lstm-data.pkl"),"rb"))
...
for i in range(3):
    ...
    plt.plot(x, real_posvel[start:end, i+6]/544, label=f"motor {i} vel", linestyle="dashed")
...
for i in range(3):
    ...
    plt.plot(x, next_sim_posvel[start:end, i+6]/2, label=f"motor {i} vel", linestyle="dashed")
    plt.plot(x, actions[start:end, i], label=f"motor {i} action", linestyle="dotted")
