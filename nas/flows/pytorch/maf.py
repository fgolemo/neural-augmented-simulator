...
Variable = lambda *args, **kwargs: autograd.Variable(*args, **kwargs).cuda() \
    if cuda else autograd.Variable(*args, **kwargs)
...
parser.add_argument('--n_epochs', type=int, default=2e4)
...
class BatchNorm(nn.Module):
    ...
    def forward(self, x, cond_y=None):
        ...
        detach_t = lambda x : x.detach().cpu().numpy()
        print('in sum log var {:6.3f} ; out sum log var {:6.3f}; sum log det {:8.3f}; mean log_gamma {:5.3f}; mean beta {:5.3f}'.format(
            detach_t((var + self.eps).log().sum()), detach_t(y.var(0).log().sum()), log_abs_det_jacobian.mean(0).item(), self.log_gamma.mean(), self.beta.mean()))
    ...
    for i, data in enumerate(dataloader):
        ...
        if i % args.log_interval == 0:
            print('epoch {:3d} / {}, step {:4d} / {}; loss {:.4f}'.format(
                epoch, args.start_epoch + int(args.n_epochs), i, len(dataloader), loss.item()))
...
def train_and_evaluate(model, train_loader, test_loader, optimizer, args):
    ...
    for i in range(args.start_epoch, args.start_epoch + int(args.n_epochs)):
        ...
        # plot sample
        if args.dataset == 'TOY':
            plot_sample_and_density(model, model.base_dist, args, step=i)
...
def plot_density(dist, ax, ranges, flip_var_order=False):
    ...
    density = dist.log_prob(Variable(xy)).exp()
    ax.contour(xx, yy, density.view(n,n).data.detach().cpu().numpy())
...
def plot_dist_sample(data, ax, ranges):
    ...
    ax.scatter(data[:,0].data.detach().cpu().numpy(), data[:,1].data.detach().cpu().numpy(), s=10, alpha=0.4)
    # format
    (xmin, xmax), (ymin, ymax) = ranges
    ax.set_xlim(xmin, xmax)
    ax.set_ylim(ymin, ymax)
...
if __name__ == '__main__':
    ...
    if args.model == 'made':
        ...
        model = MAF(args.n_blocks, args.input_size, args.hidden_size, args.n_hidden, args.cond_label_size,
                    args.activation_fn, args.input_order, batch_norm=not args.no_batch_norm).to(device)
    else:
